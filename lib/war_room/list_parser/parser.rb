class WarRoom::ListParser::Parser < Parslet::Parser
  ...
  rule(:warroom_list) {
    warroom_header >>
      ...
      dynamic { |s, c| c.captures[:totals][:warnouns][:total]       != '0' ? battlegroup_and_attachment         : str('') } >>
      dynamic { |s, c| c.captures[:totals][:battle_engines][:total] != '0' ? battle_engines.as(:battle_engines) : str('') } >>
      ...
      dynamic { |s, c| c.captures[:totals][:units][:total] != '0' ? units.as(:units) : str('') } >>
