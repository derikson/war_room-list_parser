...
class WarRoom::ListParser::Parser < Parslet::Parser
  ...
  rule(:warnoun)             { not_hyphen.as(:name) >> hyphen >> (str('WB: +') | str('WJ: +')) >> match('\d').repeat(1, nil).as(:points) >> newline }
  rule(:warnoun_attachment)  { str('-') >> space >> (match['^\r\n'] >> str('PC:').absent?).repeat(1, nil).as(:warnoun_attachment_name) >> newline }
  rule(:warbj)               { str('-') >> space >> not_hyphen.as(:warbj_name) >> hyphen >> pc }
  rule(:warbjs)              { (warbj >> newline).repeat }

  rule(:battle_engine) { not_hyphen.as(:battle_engine_name) >> hyphen >> pc }
  ...
  rule(:warroom_list) do
    warroom_header >>
      ...
      dynamic { |s, c| c.captures[:totals][:battle_engines][:total] != '0' ? battle_engines.as(:battle_engines)      : empty_match_sequence.as(:battle_engines) } >>
      ...
      dynamic { |s, c| c.captures[:totals][:units][:total] != '0' ? units.as(:units) : empty_match_sequence.as(:units) } >>
